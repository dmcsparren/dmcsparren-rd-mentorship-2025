...
export default function Inventory() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const current = newItem.currentQuantity;
    const minimum = newItem.minimumQuantity;
    ...
    createInventoryMutation.mutate({
      ...
      currentQuantity: parseInt(newItem.currentQuantity.toString()),
      minimumQuantity: parseInt(newItem.minimumQuantity.toString()),
      status,
      forecast,
      createdAt: now,
      updatedAt: now,
  ...
  const handleBarcodeDetected = (data: { name?: string; category?: string; quantity?: string; unit?: string }) => {
    ...
    setNewItem({
      ...
      quantity: data.quantity ? parseFloat(data.quantity) : newItem.quantity,
  ...
  const columns: ColumnDef<InventoryItem>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const status = row.original.status;
        const current = getNumericValue(row.original.quantity);
